  ...
  return (
    <div className="p-4 sm:p-6 lg:p-10 max-w-7xl mx-auto w-full space-y-6 font-sans">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-[#EBE6DF] pb-6">
        ...
        {/* Export Actions Bar — symmetric 2x2 grid of equal-width buttons */}
        <div className="shrink-0 w-full md:w-auto">
          <span className="block text-[10px] font-bold text-[#888888] uppercase tracking-wider mb-1.5 font-mono">
            Export Blueprint
          </span>
          <div className="grid grid-cols-2 gap-2 md:w-[248px]">
            {(['pdf', 'docx', 'ppt', 'md'] as const).map((fmt) => (
              <button
                key={fmt}
                onClick={() => handleExport(fmt)}
                disabled={!!exportingFormat}
                className="inline-flex items-center justify-center gap-1.5 px-3 py-2 rounded-xl bg-white hover:bg-[#F6F2EB] border border-[#EBE6DF] text-xs font-bold text-[#202020] hover:text-[#7A263A] hover:border-[#7A263A]/40 shadow-xs transition-all uppercase disabled:opacity-60 disabled:cursor-not-allowed"
              >
                <Download className="w-3.5 h-3.5 text-[#7A263A] shrink-0" />
                <span>{exportingFormat === fmt ? '...' : fmt}</span>
              </button>
            ))}
          </div>
        </div>
